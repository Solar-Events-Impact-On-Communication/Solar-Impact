/* =========================================================
   Solar Events — AdminPage.jsx
   Admin portal: login, events, accounts, profile, about, team
   ========================================================= */

import './AdminPage.css';
import React, { useState, useEffect, useRef, useCallback } from 'react';
import Cropper from 'react-easy-crop';
import { ADMIN_API_BASE, joinUrl, formatEventDateLabelWithYear, getEventYear } from '../../utils';

/* ---- Error Boundary (admin modal crashes) ---- */
...
export default function AdminView() {
  ...
  const [addArticleErrors, setAddArticleErrors] = useState({ file: '', caption: '' });
  ...
  const confirmDeleteMedia = async () => {
    ...
    try {
      ...
      setEventMedia((prev) => {
        ...
        if (next.length === 0) setEventMediaIndex(0);
        else setEventMediaIndex((idx) => (idx >= next.length ? next.length - 1 : idx));
        return next;
      });
  ...
  useEffect(() => {
    if (adminTab === 'team') loadAdminTeam();
  }, [adminTab]);
  ...
  const handleAddTeamMember = async () => {
    ...
    try {
      ...
      const res = await fetch(`${ADMIN_API_BASE}/api/admin/team`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ name: newMember.name.trim(), role: newMember.role.trim() }),
      });
      ...
      if (newMemberCroppedDataUrl) await handlePhotoUpload(created.id, newMemberCroppedDataUrl);
  ...
  const handleSaveTeamMember = async (member) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Failed to save member: ${res.status}`);
      await res.json();
  ...
  const changeAdminTab = (nextTab) => {
    if (adminTab === 'about' && aboutEditMode && nextTab !== 'about') handleCancelAboutEdit();
    setAdminTab(nextTab);
  };
  ...
  const handleSaveEvent = async () => {
    ...
    if (eventForm.date && eventForm.date.length === 10) {
      ...
      if (mm && dd && yyyy && yyyy.length === 4) isoDate = `${yyyy}-${mm}-${dd}`;
    }
    const payload = {
      event_date: isoDate,
  ...
  const loadAdminEvents = async () => {
    try {
      ...
      setEventsError('');
      const res = await fetch(`${ADMIN_API_BASE}/api/admin/events`, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
  ...
  const loadAdminUsers = async () => {
    try {
      ...
      setUsersError('');
      const res = await fetch(`${ADMIN_API_BASE}/api/admin/users`, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      setAdminUsers(data || []);
    } catch (err) {
  ...
  const loadAdminAbout = async () => {
    try {
      ...
      setAboutError('');
      const res = await fetch(`${ADMIN_API_BASE}/api/admin/about`, { credentials: 'include' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
    ...
    setAboutSections((prev) => {
      ...
      for (const s of prev) {
        if (s.localId === localId) {
          if (s.id) keptDeleted.push({ ...s, isDeleted: true });
        } else {
          if (s.isDeleted) keptDeleted.push(s);
          else keptActive.push(s);
        }
      }
  ...
  const handleSaveAboutChanges = async () => {
    try {
      ...
      for (const sec of sectionsToSave) {
        ...
        if (!sec.id && !payload.title && !payload.text) continue;
        if (sec.id) {
          await fetch(`${ADMIN_API_BASE}/api/admin/about/${sec.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
  ...
  const saveSecurityEdit = async () => {
    ...
    try {
      ...
      setLoggedInUser((prev) =>
        prev ? { ...prev, security_question_id: questionId || null } : prev
      );
      cancelSecurityEdit();
    } catch (err) {
  ...
  const savePasswordEdit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${ADMIN_API_BASE}/api/admin/profile`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ userId: loggedInUser.id, password: pwd }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const openCreateAccountModal = () => {
    ...
    setAccountForm({ username: '', password: '', securityQuestionId: '', securityAnswer: '' });
  ...
  const handleSaveAccount = async () => {
    try {
      ...
      if (isCreate) {
        ...
        const body = { securityQuestionId: payload.securityQuestionId };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (needsSecurity && securityAnswer) body.securityAnswer = securityAnswer;
      ...
      if (!res.ok) {
        if (data?.requiresSecurityAnswer) {
          ...
          if (needsSecurity) setError(data?.error || 'Incorrect security answer.');
          else setError('');
          setLoading(false);
          return;
        }
        ...
        <div className="admin-modal-backdrop" onClick={closeAccountModal}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className={'admin-field ' + (usernameMissing ? 'admin-field--error' : '')}>
              <div className="admin-field-label">
                <span>
                  Username{isCreateMode && <span className="admin-field-required"> *</span>}
                </span>
              </div>
              <input
            ...
            <div className={'admin-field ' + (questionMissing ? 'admin-field--error' : '')}>
              ...
              <select
                ...
                onChange={(e) =>
                  setAccountForm((prev) => ({ ...prev, securityQuestionId: e.target.value || '' }))
                }
              >
                <option value="">None</option>
                {securityQuestions.map((q) => (
            ...
            <div className={'admin-field ' + (answerMissing ? 'admin-field--error' : '')}>
              ...
              <div className="admin-password-wrapper">
                <input
                  ...
                  onChange={(e) =>
                    setAccountForm((prev) => ({ ...prev, securityAnswer: e.target.value }))
                  }
                />
                <button
                  type="button"
      ...
      <div className="admin-modal admin-event-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="admin-event-modal-body">
          <div className="admin-event-modal-form">
            {[
              { field: 'date', label: 'Date', isDate: true, placeholder: 'MM/DD/YYYY' },
              { field: 'event_type', label: 'Event Type' },
              { field: 'location', label: 'Location' },
              { field: 'title', label: 'Title' },
            ].map(({ field, label, isDate, placeholder }) => (
              <div
                key={field}
                className={
                  'admin-field ' +
                  (eventTriedSubmit && eventErrors[field] ? 'admin-field--error' : '')
                }
              >
                <div className="admin-field-label">
                  <span>
                    {label} <span className="admin-field-required"> *</span>
                  </span>
                </div>
                <input
                  className={`input ${eventTriedSubmit && eventErrors[field] ? 'input--error' : ''}`}
                  {...(isDate
                    ? { type: 'text', inputMode: 'numeric', maxLength: 10, placeholder }
                    : {})}
                  value={form[field] || ''}
                  onChange={(e) => {
                    if (isDate) {
                      ...
                      if (eventTriedSubmit)
                        setEventErrors((prev) => ({
                          ...prev,
                          [field]: isValidDate(e.target.value) ? '' : prev[field],
                        }));
                    } else {
                      ...
                      if (eventTriedSubmit)
                        setEventErrors((prev) => ({
                          ...prev,
                          [field]: v.trim() ? '' : prev[field],
                        }));
                    }
                  }}
                />
                {eventTriedSubmit && eventErrors[field] && (
                  <span className="admin-field-error-text">{eventErrors[field]}</span>
                )}
              </div>
            ))}
            {[
              { field: 'short_description', label: 'Short Description', rows: 3 },
              { field: 'summary', label: 'Summary', rows: 8 },
              { field: 'impact_on_communication', label: 'Impact on Communication', rows: 8 },
              ...
              >
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    ...
                    setForm((prev) => ({ ...prev, [field]: v }));
                    if (eventTriedSubmit)
                      setEventErrors((prev) => ({ ...prev, [field]: v.trim() ? '' : prev[field] }));
                  }}
                />
                {eventTriedSubmit && eventErrors[field] && (
                  <span className="admin-field-error-text">{eventErrors[field]}</span>
          ...
          <div className="admin-event-modal-media">
                ...
                <div className="admin-event-media-side admin-event-media-side--right">
                  ...
                  <div className="admin-event-media-side-block">
                    <button
                      ...
                      onClick={() => onOpenAddArticleModal && onOpenAddArticleModal()}
                      disabled={uploadBusy || mediaLoading}
                    >
                      +
                    </button>
            ...
            {!mediaLoading && hasMedia && (
              <div className="admin-event-caption-section">
                ...
                <textarea
                  className={`input admin-event-caption-input ${captionEditing ? 'is-editing' : 'is-readonly'}`}
                  rows={3}
